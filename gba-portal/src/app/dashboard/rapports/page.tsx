...
export default async function DashboardSynthesePage() {
  ...
  const [{ count: playersCount }, { count: teamsCount }, { data: sessions }] =
    await Promise.all([
  ...
  if (nextIds.length > 0) {
    ...
    for (const row of (att ?? []) as AttendanceRow[]) {
      ...
      attendanceCountBySession.set(
        sid,
        (attendanceCountBySession.get(sid) ?? 0) + 1,
      );
    }
  }
    ...
    <div className="grid gap-6">
        ...
        <Card className="premium-card card-shell rounded-3xl">
          ...
          <CardContent>
            ...
            <p className="mt-1 text-xs text-white/50">
              séance(s) sans saisie (sur les prochaines)
            </p>
            <div className="mt-4">
      ...
      <Card className="premium-card card-shell rounded-3xl">
        <CardHeader>
          ...
          <CardDescription>
            Référence club. Clique pour gérer les présences.
          </CardDescription>
        </CardHeader>
        <CardContent>
          {nextSessions.length === 0 ? (
            ...
            <ul className="grid gap-3 md:grid-cols-2">
              {nextSessions.map((s) => {
                const hasAttendance =
                  (attendanceCountBySession.get(String(s.id)) ?? 0) > 0;
      ...
      <p className="text-xs text-white/45">
        Note : cette page remplace l’ancien module “Rapports” (trop complexe).
        On garde uniquement du concret et actionnable.
      </p>
    </div>
  );
}
