...
export function StockClient({ initialStock }: { initialStock: StockItem[] }) {
  ...
  const [selectedItem, setSelectedItem] = React.useState<StockItem | null>(
    null,
  );
    ...
    <div className="grid gap-6">
      ...
      <div>
        ...
        <p className="mt-2 max-w-3xl text-sm text-white/70">
          Suivi inventaire temps réel : quantités, seuils mini, filtres
          (pôle/lieu/type) et mouvements.
        </p>
      </div>
      ...
      <Card className="premium-card card-shell rounded-3xl">
        <CardHeader>
          ...
          <CardDescription>
            Filtrer l’inventaire et isoler les réassorts.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="grid gap-3 md:grid-cols-4">
            ...
            <label className="grid gap-2">
              ...
              <select
                value={location}
                onChange={(e) =>
                  setLocation(e.target.value as StockLocation | "all")
                }
                className={inputBaseClassName()}
