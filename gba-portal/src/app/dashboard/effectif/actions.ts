...
export async function requestCreatePlayer(formData: FormData) {
  ...
  const legal_guardian_name =
    String(formData.get("legal_guardian_name") ?? "").trim() || null;
  const address_street =
    String(formData.get("address_street") ?? "").trim() || null;
  const address_zipcode =
    String(formData.get("address_zipcode") ?? "").trim() || null;
  const address_city =
    String(formData.get("address_city") ?? "").trim() || null;
...
export async function requestUpdatePlayer(formData: FormData) {
  ...
  if (!playerId || !teamId || !firstname || !lastname)
    throw new Error("Champs requis manquants");
