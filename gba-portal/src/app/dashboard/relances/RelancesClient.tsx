...
function buildMessage(row: Relance) {
  if (row.kind === "licence") {
    const amount =
      typeof row.amount_due_eur === "number"
        ? formatEur(row.amount_due_eur)
        : "(montant ?)";
      ...
      <div className="grid gap-6 lg:grid-cols-[1fr_1.05fr]">
        <Card className="premium-card card-shell rounded-3xl">
          <CardHeader>
            ...
            <CardDescription>
              Sélectionnez une ligne pour voir l’aperçu du message.
            </CardDescription>
          </CardHeader>
          <CardContent>
            {rows.length === 0 ? (
                    ...
                    <li key={r.id}>
                      ...
                      >
                        ...
                        <div className="mt-3 flex flex-wrap items-center gap-2">
                          <Pill variant={kindVariant(r.kind)}>
                            {kindLabel(r.kind)}
                          </Pill>
                        ...
                        {r.kind === "licence" &&
                        typeof r.amount_due_eur === "number" ? (
          ...
          <CardContent>
            {!selectedRow ? (
              <div className="rounded-2xl border border-white/10 bg-white/5 p-5">
                ...
                <p className="mt-1 text-sm text-white/65">
                  Choisissez une ligne dans le backlog.
                </p>
              </div>
            ) : (
              <div className="grid gap-4">
                <div>
                  ...
                  <p className="mt-1 text-sm text-white/70">
                    {selectedRow.team} • {selectedRow.category} •{" "}
                    {selectedRow.pole}
                  </p>
                ...
                <div>
                  ...
                  <div className="mt-3 flex flex-wrap gap-2">
                    <Button
                      ...
                      onClick={async () => {
                        try {
                          ...
                          setToast(
                            "Impossible de copier (permissions navigateur)",
                          );
                        }
                      }}
                    >
                    ...
                    <Button
                      ...
                      variant="ghost"
                      onClick={() =>
                        handleStatusUpdate(selectedRow.id, "snoozed")
                      }
                    >
                      Snoozer
                    </Button>
                    <Button
                      ...
                      variant="ghost"
                      onClick={() =>
                        handleStatusUpdate(selectedRow.id, "pending")
                      }
                    >
                      Remettre à faire
                    </Button>
