...
function computeFormatFromTeamCategory(teamCategory: string | null | undefined): GameFormat {
  ...
  if (c.startsWith('U10') || c.startsWith('U11') || c.startsWith('U12') || c.startsWith('U13'))
    return 8
  return 11
}
...
export default function TactiquePage() {
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    if (overId === 'bench') {
      ...
      setBenchSlots((prev) =>
        prev.map((s) => (s.playerId === playerId ? { ...s, playerId: null } : s))
      )
      return
    }
    ...
    setSlots((prev) => {
      ...
      setBenchSlots((prevB) =>
        prevB.map((s) => (s.playerId === playerId ? { ...s, playerId: null } : s))
      )
                      ...
                      <div key={s.id} className="relative flex flex-col items-center gap-2 group">
                        <div className="relative h-12 w-12 rounded-full border-2 border-dashed border-white/30 flex items-center justify-center bg-white/5 group-hover:border-cyan-500/50 transition-colors overflow-visible">
                          <SlotView
                            ...
                            onClear={() =>
                              setBenchSlots((prev) =>
                                prev.map((x) => (x.id === s.id ? { ...x, playerId: null } : x))
                              )
                            }
                            onSetNumber={(n) =>
                              setBenchSlots((prev) =>
                                prev.map((x) => (x.id === s.id ? { ...x, number: n } : x))
                              )
                            }
                          />
                        </div>
