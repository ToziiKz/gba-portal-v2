...
function applyDeliveryToStock(req: EquipmentRequest) {
  ...
  const updatedItems = stockItems.map((item) => {
    const bySku = req.items.find(
      (requestItem) =>
        requestItem.sku && lowered(requestItem.sku) === lowered(item.sku),
    );
    const byName = req.items.find(
      (requestItem) =>
        !requestItem.sku &&
        lowered(item.name).includes(lowered(requestItem.name)),
    );
  ...
  const lowAfter = updatedItems.filter(
    (item) => Number(item.qty) < Number(item.min ?? 0),
  ).length;
  return { lowAfter };
}
...
export function useEquipementsState() {
  ...
  const [deliverInfo, setDeliverInfo] = useState<{ lowAfter?: number } | null>(
    null,
  );
  ...
  const updateItemRow = (
    idx: number,
    patch: { name?: string; qty?: number; sku?: string },
  ) => {
    setForm((previous) => ({
      ...previous,
