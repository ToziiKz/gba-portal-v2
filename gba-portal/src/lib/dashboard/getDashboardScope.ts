...
export async function getDashboardScope(): Promise<DashboardScope> {
  ...
  const poleScope = profile.pole_scope ? String(profile.pole_scope).trim() : null
  ...
  if (directTeams) {
    assignedTeams = [...assignedTeams, ...directTeams.map(t => ({
      id: String(t.id),
      name: t.name,
      category: t.category
    }))]
  ...
  if (poleScope) {
    ...
    if (poleTeams) {
      // Avoid duplicates
      const newTeams = poleTeams.filter(pt => !teamIds.includes(String(pt.id)))
      assignedTeams = [...assignedTeams, ...newTeams.map(t => ({
        id: String(t.id),
        name: t.name,
        category: t.category
      }))]
