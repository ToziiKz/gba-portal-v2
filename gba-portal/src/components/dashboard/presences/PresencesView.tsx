...
export function PresencesView({ sessions }: Props) {
  ...
  const [selectedSession, setSelectedSession] =
    React.useState<PresenceSession | null>(null);
  ...
  return (
    <div className="grid gap-6">
      ...
      {scope.role === "coach" ? (
        <Card className="rounded-3xl border-slate-100 bg-white">
          <CardHeader>
            ...
            <CardDescription>
              Raccourcis terrain pour pointer rapidement.
            </CardDescription>
          </CardHeader>
          <CardContent>
                ...
                {myNextSessions.map((s) => (
                  ...
                  >
                    ...
                    <p className="mt-1 inline-flex items-center gap-2 text-xs text-slate-600">
                      <Clock3 className="h-3.5 w-3.5" /> {s.day} •{" "}
                      {sessionTimeLabel(s)}
                    </p>
                    <p className="mt-1 inline-flex items-center gap-2 text-xs text-slate-600">
      ...
      <Modal
        ...
        title={
          selectedSession
            ? `Présences — ${selectedSession.team?.name ?? ""}`
            : ""
        }
        description={
          selectedSession
      ...
      >
        {selectedSession?.team ? (
          ...
          <p className="text-sm text-slate-500">
            Aucune équipe liée à cette séance.
          </p>
        )}
      </Modal>
    </div>
