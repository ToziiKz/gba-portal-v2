...
export function TeamsView({ initialTeams, role = "coach" }: Props) {
  // ... (keep existing logic)
  const [teams] = React.useState(initialTeams);
            ...
            <CardContent className="p-5">
              <div className="flex items-start justify-between mb-4">
                ...
                {team.players?.[0]?.count ? (
                  <Pill variant="neutral">{team.players[0].count} joueurs</Pill>
                ) : null}
              </div>

              <div className="space-y-3 pt-3 border-t border-white/5">
                <div className="flex items-center gap-2 text-sm text-white/70">
                  <span className="w-16 text-xs uppercase tracking-wider text-white/40">
                    Coach
                  </span>
