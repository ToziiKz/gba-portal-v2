...
export function RosterChecklist({
  ...
  >({});
  const [statsMeta, setStatsMeta] = React.useState<{
    sessionsUsed: number;
  } | null>(null);
  ...
  React.useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        const data: LocalAttendance[] = ((players ?? []) as PlayerRow[]).map(
          (p) => {
        ...
        const sessionIds = ((teamSessions ?? []) as TeamSessionRow[]).map(
          (s) => s.id,
        );
        ...
        const stats: Record<
          string,
          { pct: number; presentLike: number; total: number }
        > = {};
  ...
  const handleStatusChange = (
    ...
    setRows((prev) =>
      prev.map((row) =>
        row.playerId === playerId ? { ...row, status: newStatus } : row,
      ),
    );
  };
